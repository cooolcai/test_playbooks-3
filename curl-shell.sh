...
curl -ku admin:password -H "Content-Type: application/json" -X POST -d '{"extra_vars": {"ask_variables_on_launch": true,"output": "/media/echo-oupt2.txt"}}' http://10.20.3.44/api/v2/job_templates/23/launch/ | python3 -m json.tool